...
>(({className, ...props}, ref) => (
  <div
    ref={ref}
    className={`rounded-2xl border border-[var(--card-border)] bg-[var(--card-bg)] text-[var(--foreground)] shadow-sm ${className || ''}`}
    {...props}
  />
));
...
>(({className, ...props}, ref) => (
  <h3
    ref={ref}
    className={`font-semibold text-[var(--text-primary)] text-sm tracking-tight ${className || ''}`}
    {...props}
  />
));
